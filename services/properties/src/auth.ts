...
// Creating a brand-new property/legal entity, or listing everything on the account, isn't
// meaningfully scopable (a just-created property has no units yet to scope against) — only an OWNER
// can do that. Collaborators only ever act on properties/units already in their scope.
export function requireOwner(access: AccountAccess | null): void {
  if (!access) throw new HttpError(403, "No membership on this account");
  if (access.role !== "OWNER") throw new HttpError(403, "Only the account owner can do this");
}

export function canReadProperty(access: AccountAccess | null, propertyId: string): boolean {
  if (!access) return false;
  return access.role === "OWNER" || access.propertyIds.has(propertyId);
}

export function canWriteProperty(access: AccountAccess | null, propertyId: string): boolean {
  if (!access) return false;
  if (access.role === "OWNER") return true;
  return access.role === "COLLABORATOR" && access.propertyIds.has(propertyId);
}
