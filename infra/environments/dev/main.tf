...
module "accounts_lambda" {
  ...
  zip_path              = "${path.module}/../../../services/accounts/dist/lambda.zip"
...
module "tenancies_lambda" {
  source = "../../modules/service_lambda"

  environment           = var.environment
  service_name          = "tenancies"
  zip_path              = "${path.module}/../../../services/tenancies/dist/lambda.zip"
  vpc_subnet_ids        = module.network.private_subnet_ids
  vpc_security_group_id = module.network.lambda_security_group_id
  db_secret_arn         = module.database.master_user_secret_arn
  api_id                = module.api_gateway.api_id
